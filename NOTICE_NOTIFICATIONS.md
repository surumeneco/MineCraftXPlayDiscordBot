...
`kind` は `publish` または `update`。更新時のタグ配列は送信されるが本文には使わない。認証ヘッダーは `Authorization: Bearer <NOTICE_NOTIFY_SECRET>`。Discord送信確認後のみHTTP 204を返す。認証失敗401、入力不正400、Bot接続不可503、Discord送信失敗502。本文のメンション解析を無効化し、同一イベントのDiscord nonceを固定して短期間の重複を抑える。URLを最後の行に付ける。
